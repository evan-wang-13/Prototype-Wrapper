...
RENDER_GAME_WINDOW = (
    False  # Opens a new window to render the game (Won't work on colab default)
)
...
class PPNet(nn.Module):
    ...
    def __proto_layer_l2(self, x):
        ...
        b_size = x.shape[0]
        p = (
            self.prototypes.T.view(1, PROTOTYPE_SIZE, NUM_PROTOTYPES)
            .tile(b_size, 1, 1)
            .to(DEVICE)
        )
...
for _ in range(NUM_ITERATIONS):
    ...
    if LOAD_MODEL_FROM_FILE:
        agent.online_model.load_state_dict(
            torch.load(
                MODEL_PATH + str(LOAD_FILE_EPISODE) + ".pkl",
    ...
    for i in range(NUM_PROTOTYPES):
        ...
        dist, nn_idx = knn.kneighbors(
            X=trained_prototype, n_neighbors=1, return_distance=True
        )
    ...
    model.prototypes = torch.nn.Parameter(
        torch.tensor(real_trans_x, dtype=torch.float32)
    )
    ...
    for episode in range(SIMULATION_EPOCHS):
        ...
        for step in range(MAX_STEP):
            ...
            if np.random.random_sample() < 0.04953625663766238:
